...
public class LaserMachine : MonoBehaviour {
    ...
    void OnEnable()
    {
        m_currentProperties = m_overrideExternalProperties ? m_inspectorProperties : m_data.m_properties;
            ...
            if (m_currentProperties.RandomStuff == true)
            {
                RandomStuff();
            }
        ...
        void RandomStuff() 
        {
            m_currentProperties.m_raysNumber = RandomEvenInt();
            m_currentProperties.m_rotationSpeed = Random.Range(10,30);
            m_currentProperties.m_rayWidth = Random.Range(0.08f,0.2f);
            m_currentProperties.m_rotateClockwise = RandomBool();
            m_currentProperties.m_rotate = RandomBool();
            m_currentProperties.m_intermittent = RandomBool();
            m_currentProperties.m_intervalTime = Random.Range(0.2f, 1);
                    ...
                    if (hitInfo3D.collider)
                    {
                        ...
                        if( m_assignSparks )
                        {
                            ...
                            element.sparks.transform.rotation = Quaternion.LookRotation( hitInfo3D.normal ) ;
                                if (hitInfo3D.collider.CompareTag("Player"))
